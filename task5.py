...
if not os.path.exists(MODEL_PATH):
    ...
    hist_features = hist_windowed_df.withColumn("hour_of_day", hour("window.start")) \
                                   .withColumn("minute_of_hour", minute("window.start"))

    # TODO: Create a VectorAssembler for the new time-based features.
    # Input columns should be "hour_of_day" and "minute_of_hour".
    assembler = VectorAssembler(inputCols=["hour_of_day", "minute_of_hour"], outputCol="features") # Replace None with your implementation
    train_df = assembler.transform(hist_features) # Replace None with your implementation

    # TODO: Create and train the LinearRegression model.
    # The label column is 'avg_fare'.
    lr = LinearRegression(featuresCol="features", labelCol="avg_fare") # Replace None with your implementation
    model = lr.fit(train_df) # Replace None with your implementation
...
windowed_df = parsed_stream.groupBy(window("event_time", "5 minutes", "1 minute")).agg(avg("fare_amount").alias("avg_fare")) # Replace None with your implementation

# TODO: Apply the same feature engineering to the streaming windowed data.
# Create 'hour_of_day' and 'minute_of_hour' columns.
windowed_features = windowed_df.withColumn("hour_of_day", hour("window.start")) \
                              .withColumn("minute_of_hour", minute("window.start"))

# TODO: Create a VectorAssembler for the streaming features. It must match the one from training.
assembler_inference = VectorAssembler(inputCols=["hour_of_day", "minute_of_hour"], outputCol="features") # Replace None with your implementation
feature_df = assembler_inference.transform(windowed_features) # Replace None with your implementation

# TODO: Load the pre-trained regression model from MODEL_PATH.
trend_model = LinearRegressionModel.load(MODEL_PATH) # Replace None with your implementation

# TODO: Use the model to make predictions on the streaming features.
predictions = trend_model.transform(feature_df) # Replace None with your implementation
