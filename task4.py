...
if not os.path.exists(MODEL_PATH):
    ...
    train_df = train_df_raw.withColumn("distance_km", col("distance_km").cast(DoubleType())) \
        .withColumn("fare_amount", col("fare_amount").cast(DoubleType())) # Replace None with your implementation
    ...
    assembler = VectorAssembler(inputCols=["distance_km"], outputCol="features") # Replace None with your implementation
    train_data_with_features = assembler.transform(train_df) # Replace None with your implementation

    # TODO: Create a LinearRegression model instance.
    # Set the features column to 'features' and the label column to 'fare_amount'.
    lr = LinearRegression(featuresCol="features", labelCol="fare_amount") # Replace None with your implementation

    # TODO: Train the model by fitting it to the training data.
    model = lr.fit(train_data_with_features) # Replace None with your implementation

    # TODO: Save the trained model to the specified MODEL_PATH.
    # HINT: Use the .write().overwrite().save() methods
    model.write().overwrite().save(MODEL_PATH) # Replace None with your implementation
...
assembler_inference = VectorAssembler(inputCols=["distance_km"], outputCol="features") # Replace None with your implementation
stream_with_features = assembler_inference.transform(parsed_stream) # Replace None with your implementation
